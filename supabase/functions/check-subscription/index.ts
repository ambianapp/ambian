...
const deviceSlotPriceIds = getAllDeviceSlotPriceIds();
...
serve(async (req) => {
  ...
  try {
    ...
    // Check for existing subscription in database first (handles admin-extended trials)
    ...
    // If there's an existing subscription with a valid period_end, use that (admin-extended trial)
    if (existingSub?.current_period_end && existingSub.status === "trialing") {
      trialEndDate = new Date(existingSub.current_period_end);
      isInTrial = now < trialEndDate;
      trialDaysRemaining = isInTrial ? Math.ceil((trialEndDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000)) : 0;
      logStep("Using database trial end date (admin-extended)", { trialEndDate: trialEndDate.toISOString(), trialDaysRemaining });
    } else {
      // Default: calculate trial based on user creation date
      const TRIAL_DAYS = 3;
      const userCreatedAt = new Date(user.created_at);
      trialEndDate = new Date(userCreatedAt.getTime() + TRIAL_DAYS * 24 * 60 * 60 * 1000);
      isInTrial = now < trialEndDate;
      trialDaysRemaining = isInTrial ? Math.ceil((trialEndDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000)) : 0;
      logStep("Using default trial calculation", { isInTrial, trialDaysRemaining, trialEndDate: trialEndDate.toISOString() });
    }
    ...
    if (customers.data.length === 0) {
      logStep("No Stripe customer found, using trial status");
      
      // Only upsert if there's no existing subscription or if trial status changed
      if (!existingSub || existingSub.status !== (isInTrial ? "trialing" : "inactive")) {
      ...
      return new Response(JSON.stringify({ 
        ...
        device_slots: existingSub?.device_slots || 1,
      }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      });
    ...
    if (!hasPrepaidAccess) {
      await supabaseClient
        ...
        .upsert({
          ...
          stripe_customer_id: customerId,
          status: isInTrial ? "trialing" : "inactive",
          current_period_start: isInTrial ? new Date(user.created_at).toISOString() : null,
